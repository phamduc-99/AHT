							...
							<div class="vertical-menu">
								...
								<div class="item-menu"><span>{{$category->name}}</span>
									<div class="category-fix">
										...
										<a class="btn-category btn-primary" href="{{ route('category.edit', ['category'=>$category->id])}}"><i class="fa fa-edit"></i></a>
										
										<a class="btn-category btn-danger"  onclick="return confirm('Xóa danh mục: {{$category->name}}?')" href="{{ route('category.destroy', ['category'=>$category->id])}}"><i class="fas fa-times"></i></i></a>
										@method('DELETE')
									</div>
								</div>
								@endforeach
